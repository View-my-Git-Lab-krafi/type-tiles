# To save CPU usage, you can reduce the maximum frames per second (max_fps) as desired
max_fps = 200  
...
ttk_style.theme_settings(
  "dracula", {
    "TLabel": {
      "configure": {
        "background": dracula_colors["label_bg"],
        "foreground": dracula_colors["label_fg"]
      }
    },
    "TButton": {
      "configure": {
        "background": dracula_colors["button_bg"],
        "foreground": dracula_colors["button_fg"]
      }
    },
    "TEntry": {
      "configure": {
        "background": dracula_colors["entry_bg"],
        "foreground": dracula_colors["entry_fg"]
      }
    },
    "TCheckbutton": {
      "configure": {
        "background": dracula_colors["checkbox_bg"],
        "foreground": dracula_colors["checkbox_fg"],
        "highlightbackground": dracula_colors["checkbox_highlight"]
      }
    },
  })
...
label1 = tk.Label(root,
                  text="Welcome to Run-Type-Tiles",
                  fg="white",
                  bg="#44475a")
label1.pack()
label1 = tk.Label(root,
                  text="Visit Krafi.info to Learn more",
                  fg="white",
                  bg="#44475a")
label1.pack()
###################################

ytlink_label = ttk.Label(root,
                         text="YouTube Link",
                         font=("Helvetica", 24),
                         anchor="center")
...
remove_special_chars_label = ttk.Label(root,
...
remove_special_chars_checkbox = tk.Checkbutton(
  root, variable=remove_special_chars_var)
remove_special_chars_checkbox.pack()

auto_pause_no_type_label = ttk.Label(root,
                                     text="Auto Pause No Type",
                                     anchor="center")
...
def check_input(current_word, input_word):
  ...
  for i, char in enumerate(current_word):
    if i >= len(input_word) or char != input_word[i]:
      correct = False
      if i < len(input_word):
        surface.blit(input_font.render(input_word[i], True, (255, 85, 85)),
                     (font.size(current_word[:i])[0], 0))
    else:
      surface.blit(input_font.render(char, True, (80, 250, 123)),
                   (font.size(current_word[:i])[0], 0))
  screen.blit(
    surface,
    (screen_width / 2 - surface.get_width() / 2, screen_height / 2 - 50))
  return correct
...
while not game_over:
  ...
  if first_time == True:
    ...
    first_print = font.render("Welcome to Run-Type-Tiles,", True,
                              (255, 255, 255))
    screen.blit(first_print,
                (screen_width / 2 - first_print.get_width() / 1,
                 screen_height / 8 - first_print.get_height() / 2))
  ...
  while True:
    ...
    ) - last_key_time > AUTO_PAUSE_NO_TYPE and not paused:
    ...
    input_text_surf = input_font.render(input_word, True,
                                        (255, 85, 85) if incorrect_input else
                                        (80, 250, 123))
    input_rect = input_text_surf.get_rect(center=(screen_width / 2,
                                                  screen_height / 2 + 50))
